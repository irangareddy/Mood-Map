...
public struct ScatterPlotView: View {
    ...
    public var body: some View {
        VStack {
            ...
            //                                        PointMark(
            //                                            x: .value("Day", moodEntry.description.humanReadableDateTime(), unit: .weekday),
            //                                            y: .value("Pleasant Index", moodEntry.moods.first?.toMood().happinessIndex)
            //
            //                                        )
            //                                        .annotation(position: .overlay, alignment: .center) {
            //                                            ZStack {
            //                                                Circle()
            //                                                    .fill(getMoodCategoryColor(for: moodEntry.moods.first?.toMood().category))
            //
            //                                            }.frame(width: 8, height: 8)
            //                                        }
            //                                        .symbolSize(0) // hide the existing symbol
            //                                        .foregroundStyle(by: .value("Mood", moodEntry.moods.first?.toMood().category.rawValue))
            //                                        .symbol(by: .value("Mood", moodEntry.moods.first?.toMood().category.rawValue))
            //                                        //                    .position(by: .value("Mood", moodEntry.moods.first?.toMood().category.rawValue))
            //                                        .accessibilityLabel(moodEntry.date.description)
            //                                        .accessibilityValue(moodEntry.moods.first?.toMood().category.rawValue)
            //                                    }
            //
            //                                    .frame(height: 300)
            ...
            if displayFactor == .place {
                GroupBox(label: Text("Place")) {
                    Chart(moodEntries, id: \.self) { moodEntry in
                        ...
                        ).foregroundStyle(by: .value("Place", moodEntry.place!))
